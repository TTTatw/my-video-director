...
- 提交前**逐条人工核对**本节铁律（六槽齐不齐、有没有空形容词、有没有负面词、歧义动词、词数）。**本 skill 为纯文本，不含任何脚本、不需要运行任何 linter。**
...
- i2v / 有首帧：**让用户先给图**（可选但有用），用图**锚定"产品本身"**（版型/颜色/材质/细节必须忠实，别复述、别改动）。**参考图里的模特不是"必须当主角"**——除非用户点名用那个人，否则**角色由你定义**（长相/年龄感/体型/气质/发声与否）；环境/场景/机位/光/景深/动作节拍/音频**仍须完整写**。一句话：参考图 = 产品锚；**产品之外的一切都是你要写的，只有产品本身不能动**。
